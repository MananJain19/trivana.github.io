...
const cities = [
    ...
    },
    {
        ...
        experiences: [
            ...
            { name: "College Street Book Browse", duration: "2 hours", bestTime: "Afternoon" },
...
const modalCityImage = document.getElementById('modalCityImage');
const topSpots = document.getElementById('topSpots');
const mustTryFoods = document.getElementById('mustTryFoods');
    ...
    filteredCities.forEach(city => {
        ...
        cityCard.innerHTML = `
            ...
            <div class="city-info">
                ...
                <p>${city.tagline}</p>
                <div class="tags">
                    <span class="tag historic">Historic</span>
                    <span class="tag food">Food</span>
                    <span class="tag nature">Culture</span>
                </div>
                <button class="explore-btn" data-city="${city.id}">Explore</button>
            </div>
        `;
...
function openCityModal(cityId) {
    ...
    city.spots.forEach(spot => {
        topSpots.innerHTML += `
            <div class="item-card">
                <h4>${spot.name}</h4>
                <p>${spot.highlight}</p>
                <p class="rating">${spot.rating}</p>
            </div>
        `;
    });

    // Render foods
    mustTryFoods.innerHTML = '';
    city.foods.forEach(food => {
        mustTryFoods.innerHTML += `
            <div class="item-card">
                <h4>${food.name}</h4>
                <p>${food.place ? `Best at: ${food.place}` : ''}</p>
                <p>${food.type}</p>
            </div>
        `;
    });
    ...
    city.experiences.forEach(exp => {
        uniqueExperiences.innerHTML += `
            <div class="item-card">
                <h4>${exp.name}</h4>
                <p>Duration: ${exp.duration}</p>
                <p>Best time: ${exp.bestTime}</p>
            </div>
        `;
    });
